...
- `tag_name` is now prefixed with `terraform.remote` automatically, so
  you can see the built-in value in action without a separate variable.

## Setup
```bash
cd terra_remote_state
terraform init
```
...
Create S3 Bucket 
# bucket names must be globally unique — replace with your own suffix
aws s3api create-bucket \
  --bucket my-terraform-state-kamatchiai213-2026 \
  --region us-west-2 \
...
aws s3 ls s3://my-terraform-state-yourname-2026/env:/dev/ --recursive
aws s3 ls s3://my-terraform-state-yourname-2026/env:/prod/ --recursive


prove locking works (the debugging exercise)

Open two terminals, both cd'd into the project, both in the dev workspace:

bash
# Terminal A
terraform apply -var-file=dev.tfvars
# don't type "yes" yet — leave it sitting at the confirmation prompt
bash
# Terminal B, while A is still waiting
terraform plan -var-file=dev.tfvars


cleanup, adjusted for remote state
...
aws s3 rm s3://my-terraform-state-kamatchiai213-2026 --recursive
aws s3api delete-bucket --bucket my-terraform-state-kamatchiai213-2026 --region us-west-2
